...
class NewsApp:
    def __init__(self,app):
        self.app = app
        self.app.title("NewsApp using Tkinter")
        ...
        c2 = '#FF7612'    #'#0066CC'
        font_color = 'white'
        self.title = Label(self.app, text='NewsApp', font=('Times New Roman', 30), bg=c1, fg=font_color, relief=GROOVE, pady=2, bd=12).pack(fill=X)
        
        F1 = LabelFrame(self.app,text='Category',bg=c1,fg='white',font=('Times New Roman',20,'bold'),relief=GROOVE,bd=10)
        F1.place(x=0,y=80,width=300,relheight=0.88)
        
        for i in range(len(self.newCat)):
            b = Button(F1,text=self.newCat[i].upper(),font=('Times New Roman',14,'bold'),bd=7,width=20, height=2, bg=c2, fg='white')
        ...
        newsTitle = Label(F2,text="News Area",bg=c2,fg=font_color,bd=7,relief=GROOVE,font=('Times New Roman',20,'bold')).pack(fill=X)
        scroll_y = Scrollbar(F2,orient=VERTICAL)
        self.textarea = Text(F2,yscrollcommand=scroll_y.set,font=('Lora bold',15),bg=c1,fg=font_color)
        ...
        self.b2 = Button(F2, text='Check-Weather'.upper(), font=('Times New Roman',14), bg='#FF7612', fg='white', width=20, bd=7, command=get_weather)
        self.b2.place(x=0, y=625, relwidth=1, relheight=0.1)
    
    
    def NewsArea(self, event):
        type = event.widget.cget('text').lower()
        apiKey = '288fba8951ef47a1b50f7a67822cb74c'
